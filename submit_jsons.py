...
def submit_jsons(args):
    ...
    if args.specify_samples:
        ...
        for specified_sample in specified_samples:
            if f"{specified_sample}.json" in jsons:
                ...
                print(
                    f"Warning: JSON for specified sample '{specified_sample}' not found in jsons/{args.year}"
                )
    ...
    for json_file in jsons:
        ...
        if args.year == "Run3_2024":
            ...
            submission.submit_to_condor()
            yaml_dict[json_file] = "Submitted with gfal-copy"

        else:
            submission_results = subprocess.run(
                f"fts-rest-transfer-submit -s https://fts00.grid.hep.ph.ic.ac.uk:8446 -f jsons/{args.year}/{json_file}",
                shell=True,
                capture_output=True,
                text=True,
                check=True,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--specify_samples",
        action="store_true",
        help="Specify samples to process in specify_samples.yaml",
    )
